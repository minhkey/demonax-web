...
# Helper function to generate boss location map link
generate_boss_location_link <- function() {
  if (nrow(boss_location_data) == 0) {
    return(NULL)
  }

  x <- boss_location_data$x[1]
  y <- boss_location_data$y[1]
  z <- boss_location_data$z[1]
  zoom <- 5

  hash <- sprintf("#%d,%d,%d,%d?spawn=1", x, y, z, zoom)
  url <- sprintf("/map/index.html%s", hash)

  return(sprintf(
    '<a href="%s" style="color: #cfa600; text-decoration: none; font-weight: 500; font-size: 1rem;">📍 Show boss location on map</a>',
    url
  ))
}
...
creature <- creatures |>
  filter(id == {{creature_id}})

# Check if this is a boss with a location
boss_location_data <- boss_locations |>
  filter(boss_name == creature$name)

has_boss_location <- nrow(boss_location_data) > 0

# Query 2: Creature skills
skills_data <- creature_skills |>
  filter(creature_id == {{creature_id}}) |>
...
if (has_boss_location) {
  cat('<div style="margin-bottom: 1.5rem;">\n')
  cat(generate_boss_location_link(), '\n')
  cat('</div>\n')
}
```
...
::: {.profile-container}
::: {.profile-photo}
![](../../asset/img/outfit/{{creature_race}}.png){width=50%}
:::
